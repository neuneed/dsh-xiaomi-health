...
/* AI Health Copilot (Apple Health Style Interactive Chat) */
.dsh-health-copilot-card {
  background: #1c1c1e;
...
.dsh-health-chat-input-bar:focus-within {
  border-color: #0a84ff;
  box-shadow: 0 0 0 2px rgba(10, 132, 255, 0.25);
}

.dsh-health-chat-input {
  flex: 1;
...
.dsh-health-chat-input::placeholder {
  color: #8e8e93;
}

.dsh-health-chat-send {
  width: 32px;
  height: 32px;
  border-radius: 50%;
  background: #0a84ff;
...
.dsh-health-chat-send:hover {
  background: #0071e3;
  transform: scale(1.05);
}

.dsh-health-chat-send:disabled {
  background: #3a3a3c;
  cursor: not-allowed;
  transform: none;
}

.dsh-health-jump-session {
  display: flex;
  align-items: center;
  justify-content: space-between;
  font-size: 12px;
